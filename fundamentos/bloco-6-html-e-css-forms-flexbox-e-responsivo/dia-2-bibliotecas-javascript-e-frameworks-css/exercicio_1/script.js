...
function handleSubmit(event) {
  ...
  if(validation.errorQtd === 0){
    ...
    // Se encontrou erro, formata a div que é firstchild
    let divError = document.querySelector('.error');
    divError.className = 'notification is-primary';
    // divError.style.display = 'flex';
    // divError.style.flexDirection = 'column';
    // divError.style.alignContent = 'center';
    // divError.style.flexWrap = 'wrap';
    // divError.style.justifyContent = 'space-around';
    // divError.style.alignItens = 'center';

    let elParagraphs = document.querySelectorAll('.notification p');
      for (parag of elParagraphs) {
...
function runStyles() {
  ...
  let radioHouseType = document.querySelectorAll(`[name=houseType]`);
  for (radio of radioHouseType) {
    radio.className = 'radio';
  }

  let elTextArea = document.querySelector('textarea');
  elTextArea.className = 'textarea';
